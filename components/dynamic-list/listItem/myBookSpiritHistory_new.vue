 ...
 <template>
  <view class="myBookSpiritHistory newBuGu">
    <view class="item" :class="{ itemss: item.bookStatus == 'MATCH_FAIL' }">
      <view class="right">
        ...
        <view class="rth rtht" v-if="item.bookStatus == 'MATCH_FAIL'">
          ...
          <text class="rthR rr green"
            >GUGUX{{ item.wisp ? item.wisp.costWispCoin : 0 }} 能量晶石X{{ item.wisp ? item.wisp.costAccompanyWisp : 0 }} 土地X{{
              item.wisp ? item.wisp.growthDays : 0
            }}</text
          >
        </view>
        <view class="btnBox">
    ...
    <!-- 能力晶石組件 -->
    <spiritComponenyBuy
      v-if="isShowBuyComponeny"
      @closeBuyChild="getChildBuy"
      :itemInfo="itemInfoForComponentChild"
      @decNumber="getDec"
      @addNumber="getAdd"
      @buySuccess="childBuySuccess"
    ></spiritComponenyBuy>
    <!-- 土地組件 -->
    <spiritLandBuy
      v-if="isShowLandBuy"
      @closeLandChild="getLandChildClose"
      :itemInfo="itemInfoForComponentLandChild"
      @decLandNumber="getLandDec"
      @addLandNumber="getLandAdd"
      @buySuccess="childBuyLandSuccess"
    ></spiritLandBuy>
    <!-- 提示組件 -->
