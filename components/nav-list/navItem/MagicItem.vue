<template>
	...
	>
	  <navigator :url="getNavigationUrl(item.nav || item.url)" hover-class="navigator-hover">
	      <div class="magic_item" :style="{background: item.background||'#fff'}">
...
<!-- 						<i mode="aspectFit" class="navIcon image" v-if="item.svg.icon==='export'" :style="{'color':item.svg.color}">&#xe703;</i>
					...
					<div class="title" :style="{color:item.textColor}"><span v-if="item.required===true" style="color: #F62D2D;margin-right: 2px;font-weight: bolder;">●</span>{{item.title}}</div>
	      </div>
	  </navigator>
	</div>
</template>
