			...
			<view v-if="!skeletonLoading">
				...
				<block v-if="_get(config, 'modules', []).length > 0">
					<view v-for="(item, index) in config.modules" :key="index">
						<dynamic-form :Details="Details||item.Details" v-if="_get(item, 'type') === 'autoform'" :config="{
												..._get(config.moduleData, item.key, {}),
												outStyle: getComponentStyle(item)
										 }" :formId="pageId" :srvFormData="getComponentsData(item) || ((formData||srvFormData)||{})"
							:outTitle="_get(item,'name')" :navigator="_get(item,'navigator')"
							:modal="_get(item,'modal')" :useField="_get(item,'useField',false)"
							:ifManualSubmit="ifManualSubmit" @submit="handleSubmit" />
						<view v-if="_get(item, 'type') === 'autolist'">
							<card :otherStyle="{
													...
													}" :title="_get(item,'name','')||listName" :jump="_get(_get(config.moduleData,item.key,{}),'jump',false)"
								...
								<dynamic-list :unloading="_get(_get(config.moduleData,item.key,{}),'unloading',false)"
									:config="{
									          ..._get(config.moduleData, item.key, {}),
							...
							<dynamic-list :unloading="_get(_get(config.moduleData,item.key,{}),'unloading',false)"
								:config="{
												...
												...getComponentsData(item) ? { list: getComponentsData(item) } : {},
																	outStyle: getComponentStyle(item)
												}" v-if="!_get(item,'name')&&!listName" :otherSearch="otherSearch"></dynamic-list>
						</view>
						...
						<view v-if="_get(item, 'type') === 'banner'">
							<card :otherStyle="{
											...
											}" :title="_get(item,'name','')" v-if="_get(item,'name')!==undefined&&_get(item,'name')!==''">
								<swiper-images
									:list="getComponentsData(item) ||  _get(config.moduleData, `${item.key}.banners`, [])"
									:outStyle="getComponentStyle(item)" />
							</card>
							<swiper-images v-if="_get(item,'name')===undefined||_get(item,'name')===''"
								:list="getComponentsData(item) ||  _get(config.moduleData, `${item.key}.banners`, [])"
								:outStyle="getComponentStyle(item)" />
						</view>
						...
						<view v-if="_get(item, 'type') === 'navlist'">
							<card :otherStyle="{
								...
								<nav-list :title="_get(item,'name','')" :config="{
							...
							<nav-list v-if="_get(item,'name')===undefined||_get(item,'name')===''"
								:title="_get(item,'name','')" :config="{
										list: _get(config.moduleData, `${item.key}.navList`, []),
										itemModule: _get(config.moduleData, `${item.key}.itemModule`, []),
										outStyle: getComponentStyle(item),
						...
						<box-list v-if="_get(item,'type') === 'boxList'"
							:list="_get(config.moduleData,`${item.key}.navList`,[])"></box-list>
						<search v-if="_get(item, 'type') === 'search'" :config="_get(config.moduleData,item.key,{})">
						</search>

						<!-- 新增  2021-2-25 -->
						<cell v-if="_get(item, 'type') === 'cell'"
							:param="_get(config.moduleData,`${item.key}.param`, {})"
							:days="getComponentsData(item) || 0" />
						<!-- end -->
						<!-- 步驟條組件 2021-6-29 -->
						<steps v-if="_get(item, 'type') === 'steps'"
							:list="getComponentsData(item) ||  _get(config.moduleData, `${item.key}.steps`, [])"
							:outStyle="getComponentStyle(item)"></steps>
						<c-button v-if="_get(item, 'type') === 'button'" :config="_get(config.moduleData,item.key,{})">
						</c-button>
						<avatar v-if="_get(item,'type') === 'avatar'" :item="getComponentsData(item)"
							:avatarData="_get(config.moduleData,item.key,{})"></avatar>
						<sign-smoke-box v-if="_get(item,'type')==='sign-box'" :signData="getComponentsData(item)">
						</sign-smoke-box>
						<pay v-if="_get(item,'type') === 'no-pay'" :item="getComponentsData(item)"
							:noPayData="_get(config.moduleData,item.key,{})"></pay>
						<user v-if="_get(item,'type') === 'user'" :item="getComponentsData(item)"
							:pathData="_get(config.moduleData,item.key,{})"></user>
						<!-- tab欄組件 -->
						<pageTabbar v-if="_get(item,'type') === 'pageTabbar'"
							:tabs="_get(config.moduleData,item.key,{tabs:[]}).tabs"></pageTabbar>
						<service v-if="_get(item,'type') === 'service'" :item="getComponentsData(item)"></service>
						...
						<data-show v-if="_get(item,'type') === 'dataShow'"
							...
							:unitGroup="_get(_get(config.moduleData,item.key,{}),'unitGroup',{})"></data-show>
					</view>
				</block>
				<!-- #ifdef H5 -->
		</van-skeleton>
				...
				if (url && Object.keys(this.requsetParam).length > 0) {
					...
					Object.keys(queryData).map((x, i) => {
						if (queryData[x] !== '' && this.requsetParam[x] !== '') {
							...
							queryStr += (query[x] === '' ? `${_.get(this.requsetParam, x, '')}${symbol}` :
								`${x}=${_.get(this.requsetParam, x, query[x])}${symbol}`)
						}
					})
					...
					complete: (res) => {
						...
						if (_.get(res, 'data.code') === 200) {
							...
							if (responseData && contentType === 'base64') {
								try {
									responseData = Base64.decode(responseData) ? JSON.parse(Base64.decode(
										responseData)) : {}
								} catch {}
							}
							...
							if (id) {
								that.$timeCache(`page_${id}`, resData, that.$config.cachePolicy * 24 * 60 *
									60) // config中的cachePolicy可以控制緩存策略
								// page緩存列表
