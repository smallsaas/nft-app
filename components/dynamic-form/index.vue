		...
		<van-skeleton row="20" :loading="skeletonLoading">
			...
			<view v-if="!skeletonLoading">
				...
				<view class="fields_Title" v-if="![undefined,null,''].includes(outTitle)">{{outTitle}}
					<text class="fields_jumpTitle" @click="handleJump(navigator.url)"
						v-if="navigator.title">{{navigator.title||""}}</text>
						...
						<view class="Modal-Content">
							<view class="Modal-window">
								...
								<view class="Modal-ContentBox">
									<view v-for="(item,i) in modal.components">
										...
										<view v-if="item.type === 'code'"
											:class="isFocus===i?'focus Modal-ContentBox-InputBox':'Modal-ContentBox-InputBox'">
											<input :placeholder="item.placeholder"
												placeholder-style="color:#fff;opacity:.3;font-size:28rpx"
												class="Modal-ContentBox-Input" @focus="handleFocus(i)"
												@blur="handleBlur()" @input="(e)=>handleFieldChange(item.field,e)" />
											<text
												:class="canPush?'Modal-ContentBox-code':'Modal-ContentBox-code unPush'"
												@click="handleGetCode(item.requestUrl)">{{canPush?'獲取驗證碼':+codeTime+'秒後重試'}}</text>
										</view>
										<view v-else-if="item.type === 'password'"
											:class="isFocus===i?'focus Modal-ContentBox-InputBox':'Modal-ContentBox-InputBox'">
											<input :placeholder="item.placeholder"
												placeholder-style="color:#fff;opacity:.3;font-size:28rpx"
												class="Modal-ContentBox-Input" type="password"
												@input="(e)=>handleFieldChange(item.field,e)" @focus="handleFocus(i)"
												@blur="handleBlur()" />
										</view>
										<view v-else
											:class="isFocus===i?'focus Modal-ContentBox-InputBox':'Modal-ContentBox-InputBox'">
								...
								<view class="Modal-ButtonGroup">
									<pretty-button class="Modal-btn" borderRadius="8rpx" type="gray" borderType="gray"
										:text="modal.ModalCancelText||'取消'" @click="hideModal()"></pretty-button>
									<pretty-button class="Modal-btn" borderRadius="8rpx"
										:text="modal.ModalSuccessText||'确定'" @click="handleSumbitModal()">
									</pretty-button>
								</view>
							</view>
						</view>
				...
				<view :class="_get(fields[0],'__config__.layout') === 'colFormItem'?'fields-block':''">
					<block v-for="(item, index) in fields" :key="index">
						<base-vants v-if="_get(item, '__config__.layout') === 'colFormItem'" :fields="[{...item}]"
							:form="form" :Details="Details" @change="handleChange" @clear="handleClear" />
						<view v-if="_get(item, '__config__.layout') === 'rowFormItem'"
							style="padding: 10rpx;border-radius: 5px;">
							<card :title="_get(item, '__config__.componentName')" :Style="{
											title:{
												'font':'30px',
												'textAlign':'center',
												'padding':'10rpx 0',
											}
										}" :jump="_get(item,'__config__.jump',false)" :url="config.NextNavigation||config.submittedNavigation">
								<!-- 以上card爲标題容器 -->
								<!-- <view class="line"></view> -->
								<!-- <view>{{_get(item, '__config__.componentName')}}</view> -->
								<block v-for="(k, i) in _get(item, '__config__.children', [])" :key="i">
									<base-vants v-if="_get(k, '__config__.layout') === 'colFormItem'" :fields="[{...k}]"
										:form="form" :Details="Details" @change="handleChange" @clear="handleClear" />
									<view v-else>
										<view class="form_row_title">
											{{_get(k, '__config__.componentName')}}
										</view>
										<base-vants :fields="_get(k, '__config__.children', [])" :form="form"
											:Details="Details" @change="handleChange" @clear="handleClear" />
									</view>
								</block>
							</card>
						</view>
				...
				<view class="dynamic-form-button-group"
					v-if="!Details&&(_get(config, 'formBtns', true) && fields.length > 0)">
					<button class="dynamic-form-submitButton" @click='handleSubmit'>
						保存
					</button>
					<button class="dynamic-form-backButton" @click="handleReturn">
						取消
					</button>
				</view>
				<!-- #ifdef H5 -->
		...
		methods: {
				...
				uni.request({
					...
					complete: (res) => {
						...
						if (_.get(res, 'data.code') === 200) {
							...
							that.$timeCache(`page_${that.formId}_form_Srv`, that.form, that.$config
								.cachePolicy * 24 * 60 * 60)
						}
					}
				})
			},
			...
			handleInitFormData() {
				const renderChild = (data = []) => {
					data.map(x => {
						const value = this.form[x.__vModel__] || _.get(this.srvFormData, x.__vModel__) || _
							.get(x, '__config__.defaultValue')
			...
			handleClear(e, item) {
				...
				const checkRequired = (data = []) => {
					data.map(x => {
						if (x['__vModel__'] === item['__vModel__'] && ['', undefined].includes(this.form[x
								.__vModel__])) {
							x.error = _.get(x, '__config__.required') ? !e : false
						}
						if (_.get(x, '__config__.children', []).length > 0) {
			...
			handleRecursive(data = []) {
				data.map(x => {
					if (_.get(x, '__config__.required') === true) {
						...
						if (formType === 'el-switch' && ['', undefined, null, false].includes(this.form[x
								.__vModel__])) {
							x.error = true
						}
						if (formType === 'el-slider' && ['', undefined, null, false, 0].includes(this.form[x
								.__vModel__])) {
							x.error = true
						}
					}
